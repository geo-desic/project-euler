...
namespace ProjectEuler.Problems
{
    public class Problem011 : Problem<long>
    {
        ...
        private long MaxProductInALine(long[,] array, int lineLength)
        {
            ...
            {
                var down = true;
                if (i + lineLength >= height) down = false;

                for (var j = 0; j < width; ++j)
                {
                    var right = true;
                    if (j + lineLength >= width) right = false;

                    var productDiagonal1 = 1L;
                    var productDiagonal2 = 1L;
                    var productHorizontal = 1L;
                    var productVertical = 1L;
                    ...
                    for (var k = 0; k < lineLength; ++k)
                    {
                        ...
                        {
                            //horizontal
                            productHorizontal *= array[i, j + k];
                            if (down)
                            {
                                //diagonal shaped like: \
                                productDiagonal1 *= array[i + k, j + k];

                                //diagonal shaped like: /
                                productDiagonal2 *= array[i + lineLength - 1 - k, j + k];
                            }
                        }
                        else if (down)
                        {
                            //vertical
                            productVertical *= array[i + k, j];
                        }
                    }
        ...
        private readonly long[,] grid = { { 08, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52, 12, 50, 77, 91, 08 },
